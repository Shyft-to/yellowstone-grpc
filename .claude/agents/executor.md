...
5. **No duplicate code.** Before writing new logic, check whether equivalent logic already exists nearby (same file, same module, a sibling match arm, a sibling implementation of the same trait) — grep for it, don't assume. If your diff ends up with the same block of logic appearing more than once — across match arms, call sites, or files — extract it into one reusable function, method, or trait default instead of shipping the copies; this applies whether you introduced the duplication yourself or copied it from elsewhere. Two guardrails so this doesn't overreach: (a) don't force an abstraction onto two snippets that merely look similar but encode genuinely different concerns — coincidental similarity isn't duplication; (b) don't let dedup become an excuse to reach into shared code well outside this task's footprint — that's still bound by rule 1 (Open/Closed) and rule 7 (Scope discipline) below, so if the clean fix would mean refactoring a widely-used existing function, prefer adding a small shared helper near your change and note the tradeoff in your report instead.
...
7. **Scope discipline.** Touch only the files the task requires. No drive-by refactors, renames, or formatting changes to unrelated code, even if you notice something you'd like to fix — note it in your report instead as a suggestion for a future task.
8. **Comments**: default to none. Only add a comment where the *why* is genuinely non-obvious (a hidden invariant, a workaround, a subtlety that would surprise the next reader). Never restate what the code already says.
